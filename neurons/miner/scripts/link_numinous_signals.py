...
def link_numinous_signals_impl(
    ...
    console.print(
        Panel.fit(
            "[bold cyan]Link Numinous Signals API Key[/bold cyan]",
            border_style="cyan",
            padding=(1, 2),
        )
    )
    ...
    console.print(
        Panel.fit(
            "[bold cyan]Numinous Signals API Key Setup[/bold cyan]\n\n"
            "[dim]Get your API key from:[/dim] [cyan]https://eversight.numinouslabs.io/api-keys[/cyan]\n\n"
            "[yellow]Budget:[/yellow]\n"
            "  Miner-paid: [green]Uses your Eversight credits[/green]\n"
    ...
    api_key = getpass("Enter your Numinous Signals API key: ")
    ...
    console.print("  [dim]Service:[/dim] Numinous Signals")
    console.print("  [dim]Budget:[/dim] Miner-paid (Eversight credits)")
    ...
    with console.status("[cyan]Storing credentials in backend...[/cyan]"):
        success, error_msg = _store_numinous_signals_credentials(
            env, hotkey_keypair, api_key, track
        )
    ...
    if not success:
        ...
        console.print(
            Panel.fit(
                f"[red]Failed to link Numinous Signals API key[/red]\n\n"
    ...
    console.print(
        Panel.fit(
            "[bold green]Successfully linked Numinous Signals API key![/bold green]\n\n"
            f"[dim]Hotkey:[/dim] {hotkey_keypair.ss58_address[:16]}...\n"
            f"[dim]Service:[/dim] Numinous Signals\n"
            f"[dim]Budget:[/dim] Miner-paid (Eversight credits)\n\n"
            "[yellow]Your agent runs will now use your Numinous Signals API key[/yellow]",
            border_style="green",
            padding=(1, 2),
        )
    )
...
def _store_numinous_signals_credentials(
