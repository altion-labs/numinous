...
def link_unusual_whales_impl(
    ...
    console.print(
        Panel.fit(
            "[bold cyan]Link Unusual Whales API Key[/bold cyan]",
            border_style="cyan",
            padding=(1, 2),
        )
    )
    ...
    console.print(
        Panel.fit(
            "[bold cyan]Unusual Whales API Key Setup[/bold cyan]\n\n"
            "[dim]Get your API key from:[/dim] [cyan]https://unusualwhales.com/pricing?product=api[/cyan]\n\n"
            "[yellow]Budget:[/yellow]\n"
            "  Miner-paid: [green]$0.10 per agent run[/green]\n"
    ...
    api_key = getpass("Enter your Unusual Whales API key: ")
    ...
    console.print("  [dim]Service:[/dim] Unusual Whales")
    console.print("  [dim]Budget:[/dim] Miner-paid ($0.10/run)")
    ...
    with console.status("[cyan]Storing credentials in backend...[/cyan]"):
        success, error_msg = _store_unusual_whales_credentials(env, hotkey_keypair, api_key, track)

    if not success:
        console.print()
        console.print(
            Panel.fit(
                f"[red]Failed to link Unusual Whales API key[/red]\n\n"
    ...
    console.print(
        Panel.fit(
            "[bold green]Successfully linked Unusual Whales API key![/bold green]\n\n"
            f"[dim]Hotkey:[/dim] {hotkey_keypair.ss58_address[:16]}...\n"
            f"[dim]Service:[/dim] Unusual Whales\n"
            f"[dim]Budget:[/dim] Miner-paid ($0.10/run)\n\n"
            "[yellow]Your agent runs will now use your Unusual Whales API key[/yellow]",
            border_style="green",
            padding=(1, 2),
        )
    )
...
def _store_unusual_whales_credentials(
