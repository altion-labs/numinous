...
def link_openrouter_impl(
    ...
    console.print(
        Panel.fit(
            "[bold cyan]Link OpenRouter API Key[/bold cyan]",
            border_style="cyan",
            padding=(1, 2),
        )
    )
    ...
    console.print(
        Panel.fit(
            "[bold cyan]OpenRouter API Key Setup[/bold cyan]\n\n"
            "[dim]Get your API key from:[/dim] [cyan]https://openrouter.ai/settings/keys[/cyan]\n\n"
            "[yellow]Budget:[/yellow]\n"
            "  - Miner-paid: token-based pricing (varies by model)\n"
    ...
    console.print("  [dim]Service:[/dim] OpenRouter")
    ...
    with console.status("[cyan]Storing credentials in backend...[/cyan]"):
        success, error_msg = _store_openrouter_credentials(env, hotkey_keypair, api_key, track)

    if not success:
        console.print()
        console.print(
            Panel.fit(
                f"[red]Failed to link OpenRouter API key[/red]\n\n"
    ...
    console.print(
        Panel.fit(
            "[bold green]Successfully linked OpenRouter API key![/bold green]\n\n"
            f"[dim]Hotkey:[/dim] {hotkey_keypair.ss58_address[:16]}...\n"
            f"[dim]Service:[/dim] OpenRouter\n"
            f"[dim]Budget:[/dim] Miner-paid (token-based)\n\n"
            "[yellow]Your agent runs will now use your OpenRouter API key[/yellow]",
            border_style="green",
            padding=(1, 2),
        )
    )
...
def _store_openrouter_credentials(
