...
def link_vericore_impl(
    ...
    console.print(
        Panel.fit(
            "[bold cyan]Link Vericore API Key[/bold cyan]",
            border_style="cyan",
            padding=(1, 2),
        )
    )
    ...
    console.print(
        Panel.fit(
            "[bold cyan]Vericore API Key Setup[/bold cyan]\n\n"
    ...
    api_key = getpass("Enter your Vericore API key: ")
    ...
    console.print("  [dim]Service:[/dim] Vericore")
    console.print("  [dim]Budget:[/dim] Miner-paid ($0.05/call)")
    ...
    with console.status("[cyan]Storing credentials in backend...[/cyan]"):
        success, error_msg = _store_vericore_credentials(env, hotkey_keypair, api_key, track)

    if not success:
        console.print()
        console.print(
            Panel.fit(
                f"[red]Failed to link Vericore API key[/red]\n\n"
    ...
    console.print(
        Panel.fit(
            "[bold green]Successfully linked Vericore API key![/bold green]\n\n"
            f"[dim]Hotkey:[/dim] {hotkey_keypair.ss58_address[:16]}...\n"
            f"[dim]Service:[/dim] Vericore\n"
            f"[dim]Budget:[/dim] Miner-paid ($0.05/call)\n\n"
            "[yellow]Your agent runs will now use your Vericore API key[/yellow]",
            border_style="green",
            padding=(1, 2),
        )
    )
...
def _store_vericore_credentials(
